...
rollback() {
  ...
  if [ "$TEST_MODE" != "1" ]; then systemctl restart tomcat9 >/dev/null 2>&1 || true; fi
...
decode_asset() {
  ...
  out="$2"
  kind="$3"
  src="$TMP/$(basename "$out").b64"

  curl -fLsS --retry 3 --connect-timeout 10 --max-time 90 "$url" -o "$src" || fail "não foi possível baixar $(basename "$url")"

  python3 - "$src" "$out" "$kind" <<'PY'
from pathlib import Path
import base64, re, sys
src = Path(sys.argv[1]); out = Path(sys.argv[2]); kind = sys.argv[3]
text = src.read_text(encoding="ascii", errors="ignore")
payload = "".join(re.findall(r"[A-Za-z0-9+/=]", text))
if len(payload) < 100:
    raise SystemExit("ativo base64 vazio ou incompleto")
try:
    data = base64.b64decode(payload, validate=False)
except Exception as exc:
    raise SystemExit(f"falha ao decodificar ativo: {exc}")
if kind == "png":
    if not data.startswith(b"\x89PNG\r\n\x1a\n") or b"IEND" not in data[-32:]:
        raise SystemExit("PNG inválido ou incompleto")
elif kind == "jpg":
    if not data.startswith(b"\xff\xd8\xff") or not data.endswith(b"\xff\xd9"):
        raise SystemExit("JPEG inválido ou incompleto")
else:
    raise SystemExit("tipo de ativo desconhecido")
out.write_bytes(data)
print(f"   OK: {out.name} ({len(data)} bytes)")
PY
}

if [ "$TEST_MODE" != "1" ]; then [ "$(id -u)" = "0" ] || fail "execute como root"; fi
for c in curl python3 cp rm mkdir install grep; do need "$c"; done
...
echo "[1/5] Validando ativos - nenhuma alteração no servidor"
decode_asset "$RAW/backup-cloud-logo-dark-transparent.png.b64" "$TMP/logo.png" "png"
decode_asset "$RAW/gestao-movel-login-hero.jpg.b64" "$TMP/hero.jpg" "jpg"
...
if ! python3 - "$CTX" "$LOGO" <<'PY'
from pathlib import Path
import re, sys, xml.etree.ElementTree as ET
path = Path(sys.argv[1]); logo = sys.argv[2]
...
def replace_value(match, value):
    ...
    if re.search(r'\bvalue\s*=\s*["\'][^"\']*["\']', tag):
        return re.sub(r'(\bvalue\s*=\s*)["\'][^"\']*["\']', lambda m: m.group(1) + '"' + value + '"', tag, count=1)
    close = "/>" if tag.rstrip().endswith("/>") else ">"
    body = tag.rstrip()
    body = body[:-2] if close == "/>" else body[:-1]
    return body + ' value="' + value + '"' + close
for name, value in values.items():
    pattern = re.compile(r'<Parameter\b(?=[^>]*\bname\s*=\s*["\']' + re.escape(name) + r'["\'])[^>]*?/?>', re.I)
...
:root{--bc-navy:#0B1323;--bc-dark:#111827;--bc-purple:#3E4095;--bc-purple2:#5659C7;--bc-blue:#2563EB}
...
.login-logo::before{content:"";position:fixed;inset:0;z-index:-1;background:linear-gradient(90deg,rgba(6,13,27,.16),rgba(6,13,27,.48) 48%,rgba(6,13,27,.94) 76%),url('../images/backup-cloud/gestao-movel-login-hero.jpg') center center/cover no-repeat,var(--bc-navy)}
...
if ! python3 - "$CTX" "$CSS" "$LOGO" "$HERO" <<'PY'
from pathlib import Path
import sys, xml.etree.ElementTree as ET
ctx, css, logo, hero = map(Path, sys.argv[1:])
ET.parse(ctx)
for p in (logo, hero):
    if not p.is_file() or p.stat().st_size < 1000:
        raise SystemExit(f"ativo ausente: {p}")

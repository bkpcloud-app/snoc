...
param(
    ...
    [string]$SourceRef = 'ef941256ec6ea4a8432faf2e2082af6fff6f5a22'
)
...
$BackupRoot = Join-Path 'C:\temp\DDM-SNOC-BACKUPS' ('AUTO-UPDATE-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
try {
    ...
    Write-Host '1/5 - Baixando a correcao fixada no GitHub' -ForegroundColor Cyan

    $SyncDownload = Join-Path $WorkRoot 'SINCRONIZAR-CLIENTE.ps1'
    $LauncherDownload = Join-Path $WorkRoot 'ATUALIZAR-AD-AUTOMATICO.cmd'

    Invoke-WebRequest `
        -UseBasicParsing `
        -TimeoutSec 120 `
        -Uri ($RawBase + 'SINCRONIZAR-CLIENTE-MAIN.ps1') `
        -OutFile $SyncDownload

    Invoke-WebRequest `
        -UseBasicParsing `
        -TimeoutSec 120 `
        -Uri ($RawBase + 'ATUALIZAR-AD-AUTOMATICO.cmd') `
        -OutFile $LauncherDownload
    ...
    foreach ($File in @($SyncDownload, $LauncherDownload)) {
        ...
        if ([System.IO.File]::ReadAllText($File) -match '(?i)\bicacls(?:\.exe)?\b') {
    ...
    foreach ($Pair in @(
        ...
        if (Test-Path -LiteralPath $Pair.Destination -PathType Leaf) {
            Copy-Item `
                ...
                -Destination (Join-Path $BackupRoot (Split-Path -Leaf $Pair.Destination)) `
                -Force
        }
    ...
    Write-Host '3/5 - Sincronizando cliente e atualizando o produto agora' -ForegroundColor Cyan

    & $env:ComSpec /d /c "`"$LauncherDestination`""
    $RunCode = $LASTEXITCODE
    if ($RunCode -ne 0) {
        throw "Executor automatico terminou com codigo $RunCode."
    }

    Write-Host '4/5 - Registrando a tarefa diaria das 03:00' -ForegroundColor Cyan

    $At = [datetime]::Today.Add(
        [timespan]::ParseExact(
            $ScheduleTime,
            'hh\:mm',
    ...
    $Action = New-ScheduledTaskAction `
        ...
        -Argument ('/d /c ""{0}""' -f $LauncherDestination)
    ...
    Write-Host 'CLIENTE.ps1: catalogo main com validacao SHA-256.' -ForegroundColor Green
    Write-Host 'Produto: ultima release oficial estavel.' -ForegroundColor Green
    exit 0
}
catch {
    ...
    foreach ($Destination in $Installed) {
        $Saved = Join-Path $BackupRoot (Split-Path -Leaf $Destination)

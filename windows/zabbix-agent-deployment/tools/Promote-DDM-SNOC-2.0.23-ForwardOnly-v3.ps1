...
$Filtered=@($Lines|Where-Object{$_ -notmatch '^\$S=\[regex\]::Replace\(\$S,"\(\?m\)\^Add-Order 73 '})
if($Filtered.Count -ne ($Lines.Count-1)){throw 'Nao foi possivel remover exatamente a linha invalida do materializador v2.'}
...
Write-Host 'FORWARD_ONLY_V3=PASS'

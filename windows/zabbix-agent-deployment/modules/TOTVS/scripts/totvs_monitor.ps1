...
function ConvertTo-StateCode([string]$State){
    switch($State){
        'Running'{return 1}
        'Stopped'{return 0}
        default{return 2}
...
function Read-PreviousState{
    ...
    try{
        ...
        if($raw.timestamp_utc){$state.TimestampUtc=[DateTime]::Parse([string]$raw.timestamp_utc,$InvariantCulture,[System.Globalization.DateTimeStyles]::RoundtripKind)}
        if($raw.processes){foreach($property in $raw.processes.PSObject.Properties){$state.Processes[$property.Name]=[UInt64]$property.Value.cpu_100ns}}
    }catch{}
    return $state
}
function Save-CurrentState([DateTime]$TimestampUtc,[hashtable]$Processes){
    ...
    try{
        [System.IO.File]::WriteAllText($temporaryFile,($statePayload|ConvertTo-Json -Depth 6 -Compress),(New-Object System.Text.UTF8Encoding($false)))
...
function Get-TotvsPayload([string]$TermsText){
    ...
    $terms=Get-Terms $TermsText
    $computerSystem=@(Get-WindowsData 'Win32_ComputerSystem')|Select-Object -First 1
    ...
    $allProcesses=@(Get-WindowsData 'Win32_Process')
    $processByPid=@{}
    foreach($process in $allProcesses){$processByPid[[string][UInt32]$process.ProcessId]=$process}
    $previousState=Read-PreviousState
    $nowUtc=[DateTime]::UtcNow
    $elapsedSeconds=0.0
    if($previousState.TimestampUtc){$elapsedSeconds=($nowUtc-$previousState.TimestampUtc).TotalSeconds}
    $currentCpuState=@{}
    $discovery=@()
    $servicesById=[ordered]@{}
    foreach($service in $totvsServices){
        $serviceId=ConvertTo-ServiceId ([string]$service.Name)
        ...
        if($processId -gt 0 -and $processByPid.ContainsKey([string]$processId)){
            ...
            $creationKey=ConvertTo-CreationKey $process.CreationDate
        ...
        $memoryPercent=0.0
        if($totalMemoryBytes -gt 0){$memoryPercent=([double]$workingSetBytes/[double]$totalMemoryBytes)*100.0}
        ...
        $servicesById[$serviceId]=[ordered]@{
            ...
            state=[int](ConvertTo-StateCode ([string]$service.State))
            start_mode=[int](ConvertTo-StartModeCode $service)
            pid=[UInt32]$processId
            process_name=$processName
            cpu_percent=ConvertTo-DoubleInvariant $cpuPercent
            cpu_time_seconds=ConvertTo-DoubleInvariant $cpuTimeSeconds
            memory_working_set_bytes=[UInt64]$workingSetBytes
            memory_private_bytes=[UInt64]$privateBytes
            memory_working_set_percent=ConvertTo-DoubleInvariant $memoryPercent
        }
    }
    ...
    return[ordered]@{
        ...
        host=[ordered]@{total_memory_bytes=[UInt64]$totalMemoryBytes;logical_processors=[int]$logicalProcessors;service_count=[int]$totvsServices.Count;collection_duration_ms=[int]$durationMs}
        discovery=$discovery
...
try{
    ...
    switch($Mode.ToUpperInvariant()){
        'COLLECT'{Get-TotvsPayload $Arg1|ConvertTo-Json -Depth 10 -Compress;exit 0}

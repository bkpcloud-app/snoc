...
function Export-DDMClixmlAtomic {
    ...
    if (-not (Test-Path -LiteralPath $Parent)) { New-Item -Path $Parent -ItemType Directory -Force | Out-Null }
    $Temp = $Path + '.new-' + [guid]::NewGuid().ToString('N')
    $InputObject | Export-Clixml -LiteralPath $Temp -Depth $Depth
    Move-Item -LiteralPath $Temp -Destination $Path -Force
}
...
function Get-DDMSystemInfo {
    ...
    return New-Object PSObject -Property @{
        Caption=[string]$Os.Caption; Version=$Version; ProductType=[int]$Os.ProductType; IsServer=$IsServer
        Architecture=$Arch; Domain=[string]$Cs.Domain; PartOfDomain=[bool]$Cs.PartOfDomain
        Class=$(if ($IsServer) {'SERVER'} else {'WORKSTATION'}); OsTag=$(if ($IsServer) {'WIN_SERVER'} else {'WIN_CLIENT'})
    }
}

function Get-DDMTargetAgent {
    param($SystemInfo,$Product)
    if ($SystemInfo.Architecture -eq 'ARM64' -or $SystemInfo.Architecture -eq 'IA64' -or $SystemInfo.Architecture -eq 'UNKNOWN') {
        throw "Arquitetura nao suportada pelos artefatos do produto: $($SystemInfo.Architecture)"
    }
    if ($SystemInfo.IsServer -and $SystemInfo.Version.Major -eq 6 -and $SystemInfo.Version.Minor -le 1) {
        return New-Object PSObject -Property @{ Family='AGENT1'; Architecture=$SystemInfo.Architecture; Service='Zabbix Agent'; OppositeService='Zabbix Agent 2' }
    }
...
function Convert-DDMIPv4ToUInt32 {
    ...
    foreach ($Part in $Parts) {
        $N = 0
        if (-not [int]::TryParse($Part,[ref]$N) -or $N -lt 0 -or $N -gt 255) { throw "IPv4 invalido: $Address" }
        $Value = ($Value -shl 8) -bor [uint64]$N
    }
    return [uint32]$Value
}
...
function Get-DDMCidrInfo {
    param([string]$Cidr)
    $Pair=$Cidr.Split('/')
    if ($Pair.Count -ne 2) { throw "CIDR invalido: $Cidr" }
    $Prefix=0
...
function Test-DDMCidrOverlap {
    param([string]$Left,[string]$Right)
    $L=Get-DDMCidrInfo $Left; $R=Get-DDMCidrInfo $Right
    return ($L.Network -le $R.Broadcast -and $R.Network -le $L.Broadcast)
}

function Test-DDMIPv4InCidr {
    ...
    foreach ($Adapter in @($Adapters)) {
        ...
        foreach ($Address in @($Adapter.IPAddress)) {
            if ([string]$Address -match '^\d{1,3}(\.\d{1,3}){3}$' -and [string]$Address -notmatch '^169\.254\.') {
                try { [void](Convert-DDMIPv4ToUInt32 ([string]$Address)) } catch { continue }
                $Result += New-Object PSObject -Property @{ Address=[string]$Address; HasDefaultGateway=$HasGateway; Description=[string]$Adapter.Description }
            }
        }
    }
...
function Get-DDMDetectedModules {
    ...
    foreach ($Rule in @($Product.DefaultModuleDetection)) {
        $Detected = $false
        foreach ($Pattern in @($Rule.ServicePatterns)) {
            if (@($Services | Where-Object { $_.Name -like [string]$Pattern }).Count -gt 0) { $Detected=$true; break }
        }
        if (-not $Detected) {
            foreach ($Path in @($Rule.FilePatterns)) { if (Test-Path -LiteralPath ([string]$Path)) { $Detected=$true; break } }
        }
        if ($Detected -and $Modules -notcontains [string]$Rule.Module) { $Modules += [string]$Rule.Module }
    }
...
function Select-DDMNetworkRule {
    ...
    if ($Rules.Count -eq 0) {
        if ([bool]$Client.Scope.RequireNetworkMatch) { throw 'O cliente exige rede, mas nao possui regras de rede.' }
        return $null
    }
    if (-not (Test-DDMBlank $PreferredSite)) {
        ...
        if ($SiteRules.Count -eq 0) { throw "Site explicito sem regra de rede/proxy: $PreferredSite" }
        if ($SiteRules.Count -eq 1) { return $SiteRules[0] }
        $TopPriority=($SiteRules | Measure-Object -Property Priority -Maximum).Maximum
        $Top=@($SiteRules | Where-Object { [int]$_.Priority -eq [int]$TopPriority })
        $Destinations=@($Top | ForEach-Object { '{0}|{1}|{2}' -f $_.Site,$_.Proxy,$_.Class } | Sort-Object -Unique)
        if ($Destinations.Count -ne 1) { throw "Site explicito ambiguo: $PreferredSite ($($Destinations -join '; '))" }
        return $Top[0]
    }
    $Ignored = @()
    if ($Client.Exceptions -and $Client.Exceptions.IgnoredIPv4) { $Ignored=@($Client.Exceptions.IgnoredIPv4) }
    $Ips = @(Get-DDMLocalIPv4Info | Where-Object { $Ignored -notcontains $_.Address })
    $Matches = @()
    foreach ($Rule in $Rules) {
        $Prefix = [int](([string]$Rule.Cidr).Split('/')[1])
        $Priority = if ($null -ne $Rule.Priority) { [int]$Rule.Priority } else { 0 }
        foreach ($Ip in $Ips) {
            if (Test-DDMIPv4InCidr $Ip.Address ([string]$Rule.Cidr)) {
                $Matches += New-Object PSObject -Property @{ Rule=$Rule; Ip=$Ip.Address; Gateway=[bool]$Ip.HasDefaultGateway; Priority=$Priority; Prefix=$Prefix }
            }
        }
    }
    if ($Matches.Count -eq 0) {
        if ([bool]$Client.Scope.RequireNetworkMatch) { throw "Nenhuma rede aprovada corresponde aos IPv4 locais: $(@($Ips.Address) -join ', ')" }
        return $null
    }
    $Sorted = @($Matches | Sort-Object @{Expression='Gateway';Descending=$true},@{Expression='Priority';Descending=$true},@{Expression='Prefix';Descending=$true},@{Expression='Ip';Descending=$false})
    $Best=$Sorted[0]
    $Ties=@($Sorted | Where-Object { $_.Gateway -eq $Best.Gateway -and $_.Priority -eq $Best.Priority -and $_.Prefix -eq $Best.Prefix })
    $Identities=@($Ties | ForEach-Object { '{0}|{1}|{2}|{3}' -f $_.Rule.Site,$_.Rule.Proxy,$_.Rule.ProxyActive,$_.Rule.Class } | Sort-Object -Unique)
    if ($Identities.Count -gt 1) { throw "Empate de redes com destinos diferentes: $($Identities -join '; ')" }
    return $Best.Rule
}
...
function Resolve-DDMClientIdentity {
    ...
    $Accepted=@($Client.Scope.AcceptedDomains | ForEach-Object { ([string]$_).ToLowerInvariant() })
    if ($Accepted -notcontains $SystemInfo.Domain.ToLowerInvariant()) { throw "Dominio nao permitido: $($SystemInfo.Domain)" }
    if ($SystemInfo.IsServer -and -not [bool]$Client.Scope.ServersAllowed) { throw 'Servidores nao permitidos pelo cliente.' }
    if ((-not $SystemInfo.IsServer) -and -not [bool]$Client.Scope.WorkstationsAllowed) { throw 'Workstations nao permitidas pelo cliente.' }
    if (-not (Test-DDMBlank $Client.Scope.DomainSid)) {
        try {
            $Root=[ADSI]'LDAP://RootDSE'
            $DomainEntry=[ADSI]('LDAP://' + [string]$Root.defaultNamingContext)
            ...
            if ($DomainSid -ne [string]$Client.Scope.DomainSid) { throw "SID de dominio divergente: $DomainSid" }
        } catch { throw "Nao foi possivel validar DomainSid: $($_.Exception.Message)" }
    }
    $Cluster=''; $ExplicitSite=''
    if ($Client.Exceptions -and $Client.Exceptions.ExplicitHyperVNodes -and $Client.Exceptions.ExplicitHyperVNodes.ContainsKey($env:COMPUTERNAME.ToUpperInvariant())) {
        $Cluster=[string]$Client.Exceptions.ExplicitHyperVNodes[$env:COMPUTERNAME.ToUpperInvariant()]
        if ($Client.Exceptions.ClusterSiteMap -and $Client.Exceptions.ClusterSiteMap.ContainsKey($Cluster)) { $ExplicitSite=[string]$Client.Exceptions.ClusterSiteMap[$Cluster] }
    }
    $Rule=Select-DDMNetworkRule $Client $env:COMPUTERNAME $ExplicitSite
    $Site=''; $GroupSite=''; $Proxy=''; $ProxyActive=''; $Class=$SystemInfo.Class; $Area=''; $Role=''
    ...
    if (-not $Rule) {
        if ($Client.Communication.Proxy) { $Proxy=[string]$Client.Communication.Proxy }
        elseif ($Client.Communication.Server) { $Proxy=[string]$Client.Communication.Server }
        if ($Client.Communication.ProxyActive) { $ProxyActive=[string]$Client.Communication.ProxyActive }
        elseif ($Client.Communication.ServerActive) { $ProxyActive=[string]$Client.Communication.ServerActive }
        else { $ProxyActive=$Proxy }
    }
    if (-not (Test-DDMBlank $ExplicitSite)) { $Site=$ExplicitSite }
    if (Test-DDMBlank $Proxy) { throw 'Proxy nao resolvido.' }
    $Modules=Get-DDMDetectedModules $Product
    if ($Modules -contains 'ADDS') {$Role='ADDS'} elseif ($Modules -contains 'HYPERV') {$Role='HYPERV'} elseif ($Modules -contains 'VEEAM') {$Role='VEEAM'} elseif ($Modules -contains 'MSSQL') {$Role='MSSQL'} else {$Role=$Class}
    $Base=$env:COMPUTERNAME.ToUpperInvariant(); $BaseWithout=$Base
    if ($BaseWithout.StartsWith('SRV-')) { $BaseWithout=$BaseWithout.Substring(4) }
    $Pattern=[string]$Client.Identity.HostnamePattern
    if ($Client.Identity.NormalHostnamePattern) {
        if ($Class -eq 'IND' -and $Client.Identity.IndustrialHostnamePattern) { $Pattern=[string]$Client.Identity.IndustrialHostnamePattern }
        else { $Pattern=[string]$Client.Identity.NormalHostnamePattern }
    }
    ...
    if ($Hostname.Length -gt 128 -or $Hostname -notmatch '^[A-Z0-9._-]+$') { throw "Hostname invalido: $Hostname" }
    $Metadata=Expand-DDMTokens ([string]$Client.Identity.MetadataTemplate) $Tokens
    if ($Metadata -match "`r|`n") { throw 'HostMetadata nao pode conter quebra de linha.' }
    $Bytes=[System.Text.Encoding]::UTF8.GetByteCount($Metadata)
    $Limit=if ($Client.Identity.HostMetadataMaxBytes) {[int]$Client.Identity.HostMetadataMaxBytes} else {2034}
    if ($Bytes -gt $Limit) { throw "HostMetadata excede $Limit bytes: $Bytes" }
...
function Test-DDMPortOwnedByProcess {
    ...
    foreach ($Line in $Lines) {
        if ([string]$Line -match '(\d+)\s*$') {
            try { $P=Get-Process -Id ([int]$Matches[1]) -ErrorAction Stop; if ($ProcessNames -contains $P.ProcessName.ToLowerInvariant()) { return $true } } catch { }
        }
    }
    return $false
}
...
function Get-DDMFreeSpaceMB {
    ...
    $Disk=Get-WmiObject Win32_LogicalDisk -Filter ("DeviceID='" + $Root.TrimEnd('\') + "'") -ErrorAction SilentlyContinue
    if ($null -eq $Disk) { return 0 }
    return [math]::Floor([double]$Disk.FreeSpace / 1MB)
}

...
function Get-ServiceBinaryPath([string]$PathName) {
    $Text=$PathName.Trim()
    if ($Text.StartsWith('"')) { $Text=$Text.Substring(1);$End=$Text.IndexOf('"');if($End -ge 0){$Text=$Text.Substring(0,$End)} }
    else { $End=$Text.ToLowerInvariant().IndexOf('.exe ');if($End -ge 0){$Text=$Text.Substring(0,$End+4)} }
    try { return [System.IO.Path]::GetFullPath($Text).TrimEnd('\').ToLowerInvariant() } catch { return $Text.TrimEnd('\').ToLowerInvariant() }
}

function Test-DDMTcp([string]$RemoteHost,[int]$Port,[int]$TimeoutMs=3000) {
    if (Test-DDMBlank $RemoteHost) { return $false }
    $TcpClient=New-Object System.Net.Sockets.TcpClient
    try { $Async=$TcpClient.BeginConnect($RemoteHost,$Port,$null,$null);if(-not$Async.AsyncWaitHandle.WaitOne($TimeoutMs,$false)){return $false};$TcpClient.EndConnect($Async);return $true }
    catch { return $false }
...
function Test-DDMCompliance($Target,$Identity,$Client) {
    ...
    if($null -eq $Service){$Drift+='servico_ausente'}elseif($Service.Status -ne 'Running'){$Drift+='servico_parado'}
    $ServiceInfo=Get-ServiceInfo $Target.Service
    if($ServiceInfo){
        if((Get-ServiceBinaryPath ([string]$ServiceInfo.PathName)) -ne (Get-ServiceBinaryPath $Binary)){$Drift+='caminho_servico_divergente'}
        if([string]$ServiceInfo.StartMode -ne 'Auto'){$Drift+='startup_nao_automatico'}
    }
    if($null -ne $Opposite){$Drift+='servico_oposto_instalado'}
    if($ActualVersion -ne [string]$Desired.AgentVersion){$Drift+=('versao_agente='+$ActualVersion)}
    ...
    if(-not(Test-DDMPortOwnedByProcess $ListenPort @($ExpectedProcess))){$Drift+='porta_nao_pertence_ao_agente_alvo'}
    if(-not(Test-Path $Config)){$Drift+='config_ausente'}elseif(-not(Test-AgentConfiguration $Target.Family $Binary $Config)){$Drift+='config_invalida'}

    $LastGoodPath=Join-Path $StateRoot 'last-good-state.clixml'
    if(-not(Test-Path $LastGoodPath)){$Drift+='estado_ausente'}else{
        $Good=Import-DDMClixmlSafe $LastGoodPath
        if([string]$Good.ReleaseId -ne [string]$Desired.ReleaseId){$Drift+='release_divergente'}
        if([string]$Good.ProductVersion -ne [string]$Desired.ProductVersion){$Drift+='motor_divergente'}
        if([string]$Good.AgentVersion -ne [string]$Desired.AgentVersion){$Drift+='estado_agente_divergente'}
        if([string]$Good.ClientRuntimeSha256 -ne [string]$Desired.ClientRuntimeSha256){$Drift+='cliente_divergente'}
        if([string]$Good.ClientSourceSha256 -ne [string]$Desired.ClientSourceSha256){$Drift+='cliente_fonte_divergente'}
        if([string]$Good.Hostname -ne [string]$Identity.Hostname){$Drift+='hostname_divergente'}
        if([string]$Good.Proxy -ne [string]$Identity.Proxy){$Drift+='proxy_divergente'}
        if([string]$Good.ProxyActive -ne [string]$Identity.ProxyActive){$Drift+='proxy_ativo_divergente'}
        if([string]$Good.Metadata -ne [string]$Identity.Metadata){$Drift+='metadata_divergente'}
        if(Test-Path $Config){if((Get-DDMSha256 $Config) -ne [string]$Good.GeneratedConfigSha256){$Drift+='hash_config_divergente'}}
        $Drift+=@(Test-ManagedModuleIntegrity $Good)
        if($Target.Family -eq 'AGENT2' -and [bool]$DDMProduct.InstallAgent2Plugins){
            $PluginProducts=@(Get-PluginProducts)
            if($PluginProducts.Count -ne 1){$Drift+=('quantidade_plugins='+$PluginProducts.Count)}
            $PluginVersion=Get-PluginVersion
            if($PluginVersion -ne [string]$Desired.AgentVersion){$Drift+=('plugin_real='+$PluginVersion)}
            foreach($PluginConf in @('mssql.conf','mongodb.conf','postgresql.conf')){if(-not(Test-Path (Join-Path (Join-Path $DDMProduct.Agent2Directory 'zabbix_agent2.d') $PluginConf))){$Drift+=('plugin_conf_ausente='+$PluginConf)}}
            if([string]$Good.PluginVersion -ne [string]$Desired.AgentVersion){$Drift+='plugin_estado_divergente'}
        }
    }
...
function ConvertTo-DDMJsonString([string]$Value){if($null -eq $Value){return ''};return $Value.Replace('\','\\').Replace('"','\"').Replace("`r",'\r').Replace("`n",'\n').Replace("`t",'\t')}
function ConvertTo-DDMJsonArray($Values){$Parts=@();foreach($Value in @($Values)){$Parts+='"'+(ConvertTo-DDMJsonString ([string]$Value))+'"'};return '['+($Parts -join ',')+']'}
...
function Write-ProductStatus($Client,$Identity,$Target,$Compliance,[string]$State,[string]$Message) {
    try {
        ...
        $Json='{'+
          ...
          '"drift":'+(ConvertTo-DDMJsonArray $Compliance.DriftReasons)+','+
          '"warnings":'+(ConvertTo-DDMJsonArray $Compliance.Warnings)+','+
          '"hard_blocks":'+(ConvertTo-DDMJsonArray $Compliance.HardBlocks)+','+
...
try {
    ...
    if(-not$AllowDowngrade -and $Desired.PSObject.Properties['AllowDowngrade'] -and [bool]$Desired.AllowDowngrade -and $Desired.PSObject.Properties['RollbackAuthorizationExpiresAtUtc'] -and -not(Test-DDMBlank $Desired.RollbackAuthorizationExpiresAtUtc)){
        try{if([datetime]::Parse([string]$Desired.RollbackAuthorizationExpiresAtUtc).ToUniversalTime() -gt (Get-Date).ToUniversalTime()){$AllowDowngrade=$true}}catch{}
    }
    ...
    $System=Get-DDMSystemInfo;$Target=Get-DDMTargetAgent $System $DDMProduct;$Identity=Resolve-DDMClientIdentity $Client $DDMProduct $System
    $Compliance=Test-DDMCompliance $Target $Identity $Client
    if($Compliance.HardBlocks.Count -gt 0){Write-ProductStatus $Client $Identity $Target $Compliance 'BLOCKED' ($Compliance.HardBlocks -join ',');throw "Alteracao bloqueada: $($Compliance.HardBlocks -join ', ')"}
    ...
    $State=if($Compliance.Compliant -and $Compliance.Warnings.Count -eq 0){'OK'}elseif($Compliance.Compliant){'DEGRADED'}else{'NONCOMPLIANT'}
    Write-ProductStatus $Client $Identity $Target $Compliance $State (($Compliance.DriftReasons+$Compliance.Warnings)-join ',')
    Log "Diagnostico: compliant=$($Compliance.Compliant); drift=$($Compliance.DriftReasons -join ','); warnings=$($Compliance.Warnings -join ',')"
    if($Mode -eq 'Diagnose'){Remove-OldDailyLogs;if($Compliance.Compliant){exit 0}else{exit 10}}
    if($Mode -eq 'Auto' -and $Compliance.Compliant -and -not$Force){Remove-OldDailyLogs;exit 0}
    if((Get-DDMFreeSpaceMB $StateRoot) -lt [int]$DDMProduct.MinimumFreeSpaceMB){throw 'Espaco livre insuficiente para aplicar.'}
    $Action=$Mode
    if($Mode -eq 'Auto'){
        $MsiDrift=@($Compliance.DriftReasons|Where-Object{$_ -eq 'servico_ausente' -or $_ -eq 'servico_oposto_instalado' -or $_ -like 'versao_agente=*' -or $_ -like 'plugin_real=*' -or $_ -like 'quantidade_plugins=*'})
    ...
    $Compliance=Test-DDMCompliance $Target $Identity $Client
    if($Compliance.HardBlocks.Count -gt 0){throw "Pos-validacao bloqueada: $($Compliance.HardBlocks -join ', ')"}
    if(-not$Compliance.Compliant){throw "Pos-validacao falhou: $($Compliance.DriftReasons -join ', ')"}
    ...
    $State=if($Compliance.Warnings.Count -eq 0){'OK'}else{'DEGRADED'}
    Write-ProductStatus $Client $Identity $Target $Compliance $State ($Compliance.Warnings -join ',')
    Remove-OldDailyLogs
    exit $P.ExitCode
}catch{
    try{if($Client -and $Identity -and $Target){$Dummy=New-Object PSObject -Property @{Compliant=$false;DriftReasons=@($_.Exception.Message);Warnings=@();HardBlocks=@()};Write-ProductStatus $Client $Identity $Target $Dummy 'ERROR' $_.Exception.Message}}catch{}
    Log $_.Exception.Message 'ERROR';Remove-OldDailyLogs;exit 1
}

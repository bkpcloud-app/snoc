...
switch($Mode){
    ...
    'blocked'{if(Test-Path(Join-Path $StateRoot 'release.blocked')){'1'}else{'0'}}
    'status'{$P=Join-Path $StateRoot 'product-status.json';if(Test-Path $P){[System.IO.File]::ReadAllText($P)}else{'{"state":"UNKNOWN"}'}}
